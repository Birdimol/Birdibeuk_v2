...
{ 
    ...
    public function ListerArmes()
    {
        $db = getConnexionDB();
        $requete = "SELECT * FROM arme WHERE id_user_creation = ".$this->id." ";  
        $tableau = array();
        $stmt = $db->prepare($requete);
        ...
        while ($rs = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            $tableau[]= new arme($rs['ID'], $rs['NOM'], $rs['NOM_COURT'], $rs['PRIX'], $rs['PI'], $rs['RUP'], $rs['AT'], $rs['PRD'], $rs['COU'], $rs['INT'], $rs['CHA'], $rs['AD'], $rs['FOR'], $rs['SPECIAL'], $rs['JET'], $rs['qualite'], $rs['type'], $rs['deuxmains'],$rs['debase'], $rs['id_user_creation']);
        }  
    ...
    {
        $db = getConnexionDB();
        $requete = "SELECT * FROM equipement WHERE id_user_creation = ".$this->id." ";  
        $tableau = array();
        $stmt = $db->prepare($requete);
        $stmt->execute();        
        while ($rs = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            $tableau[] = new equipement($rs['ID'], $rs['NOM'], $rs['SPECIAL'], $rs['PO'], $rs['PA'], $rs['PC'], $rs['COU'], $rs['INT'], $rs['CHA'], $rs['AD'], $rs['FO'], $rs['munition'], $rs['debase'], $rs['type'], $rs['AT'], $rs['PRD'], $rs['id_user_creation']);
        }          
        return $tableau;
    }
    
    public function ListerProtections()
    {
        $db = getConnexionDB();
        $requete = "SELECT * FROM protection WHERE id_user_creation = ".$this->id." ";  
        $tableau = array();
        $stmt = $db->prepare($requete);
        $stmt->execute();        
        while ($rs = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            $tableau[] = new protection($rs['ID'], $rs['NOM'], $rs['NOM_COURT'], $rs['PR'], $rs['RUP'], $rs['COU'], $rs['INT'], $rs['CHA'], $rs['AD'], $rs['FOR'], $rs['SPECIAL'], $rs['AT'], $rs['PRD'], $rs['PRIX'], $rs['QUALITE'], $rs['TYPE'], $rs['debase'], $rs["id_user_creation"]);
        }          
        return $tableau;
    }
    ...
    */
    ...
    public function AAccesACompagnie($compagnie)
    {
        if(!is_object($compagnie))
        {
            $compagnie = new Compagnie($compagnie);
        }
        ...
        $requete = "SELECT * FROM compagnie WHERE id_compagnie = ".$compagnie->id_compagnie." AND id_createur = ".$this->id;    
        $stmt = $db->prepare($requete);
        $stmt->execute();
